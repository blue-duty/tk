...
import {Affix, Button, Drawer, Form, Input, message, Modal} from 'antd'
import qs from "qs";
...
class AppAccess extends Component {
    ...
    async componentDidMount() {
        ...
        let applyId = urlParams.get('applicationId');
        let session = await this.createSession(applyId);
        if (!session) {
            return;
        }
    ...
    updateSessionStatus = async (sessionId) => {
        let result = await request.post(`/applic/${sessionId}/access`);
        if (result.code !== 1) {
    ...
    async createSession(assetsId) {
        let result = await request.post(`/applic/sc?applicationId=${assetsId}`);
        if (result['code'] !== 1) {
